...
<xsl:template match="/">
    ...
    function showImage(page)
    {
		...
		<div id="StaticContentImg">
			<h3 id="communityHeading" class="pageHeading">Community Events</h3>
		<xsl:for-each select="ExpressLane/ArrayOfStaticContent/StaticContent">
				<a>
					<xsl:attribute name="href">javascript:showImage(<xsl:value-of select="SortBy"/>);</xsl:attribute>
					<xsl:value-of select="SortBy"/></a><xsl:if test="position()!=last()"> | </xsl:if>
		</xsl:for-each>
			...
			<xsl:when test="ExpressLane/ArrayOfStaticContent/StaticContent[1]/Image !=''">
				<img id="Image">
					<xsl:attribute name="src">
						<xsl:value-of select="ExpressLane/ArrayOfStaticContent/StaticContent[1]/Image"/>
					</xsl:attribute>
				</img>
			</xsl:when>
			<xsl:otherwise>
				<h4>There are no community events at this time.</h4>
			</xsl:otherwise>
		</xsl:choose>
		</div>
	</xsl:template>
