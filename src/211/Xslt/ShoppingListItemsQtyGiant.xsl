...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/" name="ShoppingListItems">
		...
		<span id="ItemCount">You have <xsl:value-of select="ExpressLane/GenericShoppingList/Total"/>
			<xsl:choose>
	...
	<xsl:template match="GenericShoppingListItem" name="QuantityItem">
		...
		<tr>
			...
			<td width="20">
				<div alt="Remove Item" id="trashcanSmall">
					<xsl:attribute name="onclick">RemoveFromShoppingListByID(<xsl:value-of select="ItemID" />, <xsl:value-of select="ItemTypeID" />);
					</xsl:attribute>
				</div>
			</td>
			<td>
