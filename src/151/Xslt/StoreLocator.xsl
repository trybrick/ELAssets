...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:template match="/">
		...
		<script type="text/javascript">
			var selectedStoreId = '<xsl:value-of select="//GenericConsumer/PrimaryStoreID"/>';
			...
			function initialize() {
				...
				if (location.search == "?hold=true" || selectedStoreId != '') return;
				try {
					if ([214,215,216,217,218,225].indexOf(stores[0].ChainID) > -1){
						alert('We need a little more information from you to get you to the right page. Please select your favorite store location and you’ll be on your way!');
					}
				} catch(e) {}
    		}
			...
			**************************************************************-->
			<div id="StoreLocatorZipLookup">
				...
				<div id="storeZipEntryForm">
					<h2>
						<xsl:choose>
							<xsl:when test="string-length(//ChainSettings/Setting[SettingID=274]/Value) &gt; 0">
				...
				<div id="GoogleMapStoreList">
					<xsl:for-each select="//ArrayOfGenericStore/GenericStore[not(StoreID = //StoreSettings[string-length(StoreGroup) &gt; 0]/StoreID)]">
		...
		<xsl:variable name="LinkText">
			<xsl:choose>
				<xsl:when test="//StoreSettings[StoreID = $StoreID]/IsOGS = 'true'">
					<xsl:choose>
						<xsl:when test="string-length(//ChainSettings/Setting[SettingID=265]/Value) &gt; 0">
				...
				<xsl:otherwise>
					<xsl:choose>
						...
						<xsl:otherwise>Select Store</xsl:otherwise>
					</xsl:choose>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
	...
	<xsl:template match="node()" name="Property">
		...
		<xsl:text>: </xsl:text>
		<xsl:variable name="escaped-node">
		...
		<xsl:choose>
			...
			<xsl:otherwise>"<xsl:value-of select="normalize-space($escaped-node)"/>"</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
