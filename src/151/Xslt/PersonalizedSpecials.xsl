...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:import href="/Assets/Common/Xslt/IconLegend.xsl"/>
	...
	<xsl:template match="/">
		...
		<xsl:variable name="PageHeaderText">
			<xsl:choose>
				...
				<xsl:otherwise>My Specials</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<h2 class="pageHeader"><xsl:value-of select="$PageHeaderText" /><span> View items chosen for me based upon previous purchases.</span></h2>
					...
					<div class="couponsFrame">
						...
						<div class="couponsImage">
							<img alt="">
								...
								<xsl:attribute name="onclick"><xsl:value-of select="concat('AddToShoppingList(', CircularItemID, ',8);')"/></xsl:attribute>
								<xsl:attribute name="src"><xsl:value-of select="SmallImageUrl"/></xsl:attribute>
							</img>
						</div>
						<a class="button">
							<xsl:attribute name="href"><xsl:value-of select="concat('javascript:AddToShoppingList(', CircularItemID, ', 8);')"/></xsl:attribute>
