...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="/">
    <xsl:variable name="IsLoggedIn" select="//GenericConsumer/IsLoggedIn" />
		...
		<xsl:variable name="IsYouTechChain" select="string-length(//GenericChain/ChainSettings/Setting[SettingID=315]/Value) &gt; 0" />
		...
		<div id="Coupons">
          ...
          <xsl:element name="a">
            ...
            <xsl:attribute name="class">gridStyleIcon1<xsl:if test="$GridStyle = 1"> active</xsl:if></xsl:attribute>
            <xsl:attribute name="onclick">swapGridStyles(1);</xsl:attribute>
          </xsl:element>
          <xsl:element name="a">
            <xsl:attribute name="href">#</xsl:attribute>
            <xsl:attribute name="id">swapGrid2</xsl:attribute>
            <xsl:attribute name="class">gridStyleIcon2<xsl:if test="$GridStyle = 2"> active</xsl:if></xsl:attribute>
            <xsl:attribute name="onclick">swapGridStyles(2);</xsl:attribute>
          </xsl:element>
          <xsl:element name="a">
            <xsl:attribute name="href">#</xsl:attribute>
            <xsl:attribute name="id">swapGrid3</xsl:attribute>
            <xsl:attribute name="class">gridStyleIcon3<xsl:if test="$GridStyle = 3"> active</xsl:if></xsl:attribute>
            <xsl:attribute name="onclick">swapGridStyles(3);</xsl:attribute>
          </xsl:element>
          <xsl:element name="a">
            <xsl:attribute name="href">/Loyalty.aspx</xsl:attribute>
				...
				<div class="couponsContainer Coupon:{CouponID}">
					...
					<div id="couponsFrame{CouponID}">
						...
						<a id="removebutton{CouponID}" class="button added" href="javascript:void(0);">
							...
							<xsl:attribute name="onclick">unhighlightCoupon(<xsl:value-of select="CouponID"/>);RemoveFromShoppingListByID(<xsl:value-of select="CouponID"/>, <xsl:value-of select="$CouponTypeID" />);</xsl:attribute>
							<span>Remove</span>
						</a>
