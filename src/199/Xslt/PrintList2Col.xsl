<?xml version="1.0"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.0">
  ...
  <xsl:template match="/">
    ...
    <div id="PrintList">
      <xsl:if test="ExpressLane/GenericShoppingList/Items/GenericShoppingListItem[ItemTypeID!=10][IsVisible='true']">
        <table width="100%" style="border-bottom:3px solid black; margin-bottom:12px;">
          <tbody>
            <tr>
              <td vAlign="middle">
                <img id="logo">
                  <xsl:attribute name="src">
                    <xsl:value-of select="ExpressLane/GenericChain/DomainName"/>/Assets/<xsl:value-of select="//GenericChain/ChainID"/>/Images/printlogo.gif
                  </xsl:attribute>
                </img>
              </td>
              <td vAlign="middle">
              ...
              <td>
                <a href="javascript:alert('hi');window.print();" class="button" style="float:right;">
                  <span>Print</span>
                </a>
              </td>
            </tr>
        ...
        <xsl:variable name="CategoriesIDWithoutDigCoup" select="//GenericShoppingList/VisibleItems/GenericShoppingListItem[ItemTypeID != 13]/CategoryID"/>
        <xsl:variable name="CategoriesWithoutDigCoup" select="ExpressLane/GenericShoppingList/Categories/Category[CategoryID=$CategoriesIDWithoutDigCoup]"/>
        <xsl:variable name="test" select="$CategoriesWithoutDigCoup+$CategoriesWithoutDigCoup"/>
        <xsl:variable name="DigCouponsExists">
          <xsl:choose>
            <xsl:when test="//GenericShoppingList/VisibleItems/GenericShoppingListItem[ItemTypeID = 13]">
        ...
        <div id="column1">
          <xsl:if test="//GenericShoppingList/VisibleItems/GenericShoppingListItem[ItemTypeID = 13]">
            <div id="categoryname">
              Digital Coupons
            </div>
            <xsl:call-template name="ShoppingListCouponDetail"/>
          </xsl:if>
          <xsl:for-each select="$CategoriesWithoutDigCoup[position() mod 2 = substring($DigCouponsExists,1,1)]">
        ...
        <div id="column2">
          <xsl:for-each select="$CategoriesWithoutDigCoup[position() mod 2 = substring($DigCouponsExists,2,1)]">
            <xsl:sort select="CategoryName"/>
            <div id="categoryname">
              <xsl:value-of select="CategoryName"/>
            </div>
        ...
        <div class="StaticContent" style="clear:left; float:left; width:100%;">
      ...
      <xsl:for-each select="key('CatID',  $ShoppingListCatID)">
        ...
        <li>
          ...
          <div id="listcontent" style="text-transform:capitalize;">
            <xsl:value-of select="Quantity"/><xsl:if test="number(Weight)">
              <xsl:variable name="Weight" select="number(Weight)"/>- <xsl:value-of select="VariableWeight/GenericFraction[StoredValue = $Weight]/DisplayValue"/><xsl:choose>
            ...
            <xsl:choose>
              <xsl:when test="Price&gt;0">
                ...
                </xsl:if>$<xsl:value-of select="format-number(Price, '#.00')"/>
              </xsl:when>
  ...
  <xsl:template name="ShoppingListCouponDetail" match="ExpressLane/GenericShoppingList/VisibleItems/GenericShoppingListItem">    
    <xsl:variable name="lower">abcdefghijklmnopqrstuvwxyz</xsl:variable>
    <xsl:variable name="upper">ABCDEFGHIJKLMNOPQRSTUVWXYZ</xsl:variable>
    <ul>
      <xsl:for-each select="ExpressLane/GenericShoppingList/VisibleItems/GenericShoppingListItem[ItemTypeID=13]">
        <xsl:sort select="Description"/>
        <li>
          <div id="littlebox"/>
          <div id="couponicon">           
