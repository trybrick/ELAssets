...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:choose>
      <xsl:when test="Categories/Category">
          ...
          <div class="category">
            <table>
              ...
              <tbody>
                ...
                <xsl:for-each select="//GenericShoppingList/VisibleItems/GenericShoppingListItem[ItemTypeID = 13]">
        ...
        <xsl:for-each select="Categories/Category">
          ...
          <xsl:if test="//GenericShoppingList/VisibleItems/GenericShoppingListItem[CategoryID=$CatID and ItemTypeID != 13 ]">
            <div class="category">
  ...
  <xsl:template name="ShoppingListItemManageList" match="Category">
    ...
    <table>
      <xsl:choose>
        <xsl:when test="//GenericShoppingList/VisibleItems/GenericShoppingListItem[ItemTypeID = 13]"></xsl:when>
        <xsl:otherwise>
      ...
      <tbody>
        ...
        <xsl:for-each select="//GenericShoppingList/VisibleItems/GenericShoppingListItem[CategoryID = $CategoryID and ItemTypeID != 13 ]">
          <xsl:sort select="Description" data-type="text"/>
          <xsl:choose>
          ...
          <xsl:when test="(ItemTypeID = 3) and (string-length(ImageUrl) &gt; 0)">
            <a>
              <xsl:attribute name="href">
                javascript:RunProductDescription(<xsl:value-of select="ItemID"/>);
              </xsl:attribute>
        ...
        <xsl:if test="//GenericConsumer/IsLoggedIn = 'true'">
          ...
          <input>
            ...
            <xsl:attribute name="onkeypress">
              var key = event.keyCode ? event.keyCode : event.which; if (key == 13){ SetListComment('<xsl:value-of select="$id"/>');} return key!=13;
            </xsl:attribute>
            <xsl:attribute name="onblur">
              javascript:SetListComment('<xsl:value-of select="$id"/>')
            </xsl:attribute>
            ...
            <div>
              ...
              <div id="ProductPlusMinusButtons">
                <a class="ProductAddToListPlus">
                  <xsl:attribute name="href">
                    javascript:changeQuantity('<xsl:value-of select="$QuantitySelectID"/>', 1, 99);
                  </xsl:attribute>
                </a>
