...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
  ...
  <xsl:import href="/Assets/Common/Xslt/WeeklyAd/WeeklyAdPage.xsl"/>
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="PageHeaderDescription">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="concat('Click on an item to add to your ',$CartOrList,'.')" />
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <h2 class="pageHeader">
    ...
    <xsl:if test="count(//ArrayOfGenericStore/GenericStore) &gt; 1">
      <p class="validThruDates">
        Circular<xsl:if test="count($CircularTypes) &gt; 1">s</xsl:if> valid for <strong>
          <xsl:value-of select="//StoreCircular/Store/StoreName" disable-output-escaping="yes"/>
        </strong> location only.
      </p>
    </xsl:if>
    <span class="text-only-link">
      <xsl:if test="count(//StoreCircular/Pages/StoreCircularPage/Items/StoreCircularItem) &gt; 1">
        <a href="/Shop/WeeklyAdTextOnly.aspx">
          Text Only Version<br/>All Sale Items
        </a>
      </xsl:if>
    </span>
              ...
              <div id="CircularLists">
                ...
                <xsl:if test="$ShowGlobalSearch != 'true'">
                  ...
                  <div id="PageIndex" class="group">
                    <xsl:if test="//StoreCircular/DefaultContent/PageNumber">
                  ...
                  <div id="PageDisplay" class="group">
                    <xsl:if test="not(//StoreCircular/DefaultContent/PageNumber)">
                      <xsl:attribute name="style">display:none;</xsl:attribute>
					</xsl:if>
                      <a>
                        <xsl:attribute name="class">backtoweeklyad</xsl:attribute><xsl:attribute name="href">javascript:ShowPageList();</xsl:attribute>Choose Another Ad
                      </a>
                    ...
                    <div id="AdContent" class="group">
                      <xsl:if test="//StoreCircular/DefaultContent/PageNumber">
                        <xsl:variable name="pgNumber" select="//StoreCircular/DefaultContent/PageNumber"/>
						<xsl:for-each select="//DefaultContent">
          ...
          <xsl:for-each select="//StoreCircular/Pages/StoreCircularPage[CircularTypeID=$CircularTypeID]">
            <xsl:if test="position() = 1">
              <li>
                ...
                <small>
                  Ad Prices Valid From<br /><xsl:value-of select="concat(substring($sdate, 6,2),'/',substring($sdate, 9, 2),'/',substring($sdate, 1,4))"/> - <xsl:value-of select="concat(substring($edate, 6,2),'/',substring($edate, 9, 2),'/',substring($edate, 1,4))"/>
                </small>
                <script>
  ...
  <xsl:template match="StoreCircularType" name="PagerList">
    ...
    <xsl:param name="ID"/>
	<xsl:variable name="crPageId" select="//StoreCircular/DefaultContent/CircularPageID"/>	
    <xsl:if test="//StoreCircular/Pages/StoreCircularPage[CircularTypeID=$CircularTypeID]">
      <xsl:for-each select="//StoreCircular/Pages/StoreCircularPage[CircularTypeID=$CircularTypeID]">
        <xsl:sort select="PageNumber" data-type="number"/>
        <li>
		  <xsl:attribute name="style">
            <xsl:if test="$IsVisible = 'false'">display: none;</xsl:if>
			cursor:pointer;
          </xsl:attribute>
          <xsl:attribute name="id">
          ...
          <xsl:attribute name="class">
            <xsl:if test="CircularPageID=$crPageId">activePage</xsl:if>
          </xsl:attribute>
          <span>
  ...
  <xsl:template name="Pager" match="CircularTypes">
    <xsl:param name="ID"/>
	<xsl:choose>
         <xsl:when test="//StoreCircular/DefaultContent/PageNumber">
           <xsl:variable name="crTypeId" select="//StoreCircular/DefaultContent/CircularTypeID"/>
			<ul class="CircularPager">
			  <xsl:attribute name="id">
				...
				<xsl:call-template name="PagerList">
				  ...
				  <xsl:with-param name="CircularTypeID" select="$crTypeId"/>
				  <xsl:with-param name="IsVisible">true</xsl:with-param>
				</xsl:call-template>
				<xsl:for-each select="StoreCircularType[CircularTypeID != $crTypeId]">
					<xsl:sort select="CircularTypeID" data-type="number"/>
					<xsl:call-template name="PagerList">
					  <xsl:with-param name="ID" select="$ID"/>
					  <xsl:with-param name="IsVisible">false</xsl:with-param>
					</xsl:call-template>
				 </xsl:for-each>
			</ul>
         </xsl:when>
