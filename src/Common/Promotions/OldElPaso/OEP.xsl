...
<!--========================================================================================== -->
	...
	<xsl:template name="ProductRow">
		...
		<div id="{concat('row-',$Row)}">
			<xsl:attribute name="class">oep-product-row group<xsl:choose>
				<xsl:when test="count(//ProductList/Product[Layout/@row = $Row]) &lt; 2"> one-column</xsl:when>
				<xsl:otherwise> two-column</xsl:otherwise></xsl:choose></xsl:attribute>
...
<!--========================================================================================== -->
	...
	<xsl:template name="ProductDetail" match="Product">
		...
		<div>
			...
			<a class="oep-button" href="#null" title="Add Item to List">
				<xsl:attribute name="onclick">clickThru('%c',<xsl:value-of select="//DepartmentID" />,'<xsl:value-of select="//Brand" />','<xsl:value-of select="Title" />', '<xsl:value-of select="//ManufacturerCode" />', 1, '','','','','', <xsl:value-of select="$AdID" />, <xsl:value-of select="$CreativeID" />)</xsl:attribute>
