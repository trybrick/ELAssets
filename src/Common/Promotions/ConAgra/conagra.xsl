<!-- DWXMLSource="../../../../../Public/Sargento/Sargento2/sargento2.xml" --><xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
...
<!--========================================================================================== -->
	...
	<xsl:template name="ProductRow">
		...
		<div id="{concat('row-',$Row)}">
			<xsl:attribute name="class">sargento-product-row group<xsl:choose>
				<xsl:when test="count(//ProductList/Product[Layout/@row = $Row]) &lt; 3"> two-column</xsl:when>
...
<!--========================================================================================== -->
	...
	<xsl:template name="ProductDetail" match="Product">
		...
		<div>
			...
			<a class="conagra-button" title="Get Coupon" src="#null" target="_blank">
				<xsl:attribute name="href"><xsl:value-of select="LinkURL" /></xsl:attribute>
				<xsl:attribute name="onclick">changeSrc('<xsl:value-of select="ClickCode" />'); </xsl:attribute>
				<xsl:text>Get Coupon</xsl:text>
			</a>
		</div>
	</xsl:template>
