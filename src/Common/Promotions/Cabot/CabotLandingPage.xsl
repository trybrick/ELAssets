	...
	<xsl:template match="/">		
		...
		<div id="promoLP-wrap">
			...
			<div id="promoLP-main">
				...
				<div id="promoLP-bottom-graphic">
					<img alt="100% of Profits go to our Cabot Farmers" src="{concat(//ImagePath,//FooterImage)}"/>
				</div>
			</div>
...
<!--========================================================================================== -->
	...
	<xsl:template name="ProductDetail" match="Product">
		...
		<div class="promoLP-product-listing">		
			...
			<xsl:if test="(AddToList = 'false')">
				<a class="promoLP-button">
					<xsl:attribute name="href">
					 	<xsl:value-of select="concat(ClickCode,LinkTo)"/>
					</xsl:attribute>
