...
<!--========================================================================================== -->
	...
	<xsl:template name="ProductDetail" match="Product">
		...
		<div class="promoLP-product-listing">
			<xsl:if test="position()=last() or position=()=last()-1"><xsl:attribute name="class">last-item</xsl:attribute</xsl:if>		
			<p><xsl:value-of select="SubHeading" /></p>
			<a class="promoLP-button" title="Add Item to List" src="#null">
				<xsl:attribute name="onclick">changeSrc('<xsl:value-of select="ClickCode" />'); clickThru('%c',<xsl:value-of select="//DepartmentID" />,'<xsl:value-of select="//Brand" />','<xsl:value-of select="SubHeading" />', '<xsl:value-of select="//ManufacturerCode" />', 1, '','','','','', <xsl:value-of select="$AdID" />, <xsl:value-of select="$CreativeID" />);</xsl:attribute>
