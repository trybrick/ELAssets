...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="ArrayOfCategory" name="CategoryList">
		<xsl:call-template name="HierarchyScript" />
		<div class="CategoryHierarchy">
			<xsl:for-each select="Category[string-length(ParentCategoryID) = 0]">
				<xsl:sort select="CategoryName" data-type="text" />
				<xsl:call-template name="CategoryListElement">
					<xsl:with-param name="Layer" select="'0'" />
				</xsl:call-template>
			</xsl:for-each>
		</div>
	</xsl:template>
	<xsl:template name="HierarchyScript">
		...
		<![CDATA[
			...
			function DeselectAll() {
				...
				var categoryLinks = $$('.CategoryHierarchy a');
	...
	<xsl:template match="Category" name="CategoryListElement">
		<xsl:param name="CategoryID" select="CategoryID" />
		<xsl:param name="Name" />
		<xsl:param name="CategoryName" select="CategoryName" />
		<xsl:param name="ChildCategories" select="//Category[ParentCategoryID = $CategoryID]" />
...
<metaInformation>
	<scenarios>
		<scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="..\..\43\XmlData\43-PageTypeId-119-SpecialsPage.xml" htmlbaseurl="" outputurl="" processortype="saxon8" useresolver="no" profilemode="0" profiledepth=""
