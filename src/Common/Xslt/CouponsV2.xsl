...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="/">
		...
		<script type="text/javascript">function highlightCoupon(couponID){
			...
			states=new Array();
			<xsl:for-each select="ExpressLane/DisplayCoupons/ArrayOfCategory/Category">states[<xsl:value-of select="position()"/>]="leftCategory<xsl:value-of select="CategoryID"/>";</xsl:for-each>function highlightCategory(showit) { 
				var showcat = $(showit); 
