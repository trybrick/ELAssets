...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:import href="ShoppingList/OGSManageListTable.xsl"/>
	...
	<xsl:template match="/">
		...
		<script type="text/javascript">function LoadInternalAd(){
				var staticpagetypeid = <xsl:value-of select="ExpressLane/StaticPageType/StaticPageTypeID"/>;
					...
					<xsl:choose>
						<xsl:when test="//GenericShoppingList">
							<xsl:for-each select="//GenericShoppingList">
								<xsl:call-template name="OGSManageList"/>
							</xsl:for-each>
						</xsl:when>
						<xsl:otherwise>
