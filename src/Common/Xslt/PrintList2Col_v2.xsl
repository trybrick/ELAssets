				...
				<tbody>
					<tr>
						...
						<td vAlign="middle">
							...
							<div style="font-weight: bold; font-size: 24px; font-family: verdana">
							<xsl:variable name="IsOGS" select="//GenericConsumer/ConsumerSettings/Setting[Name='OGSConsumer']/Value" />
							<xsl:choose>
								<xsl:when test="$IsOGS = 'true'">My Shopping Cart</xsl:when>
								<xsl:otherwise>My Shopping List</xsl:otherwise>
							</xsl:choose>
							</div>
