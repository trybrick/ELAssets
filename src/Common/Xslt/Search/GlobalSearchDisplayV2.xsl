...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:import href="../GenericProduct/GenericProductDisplayV2.xsl"/>
	...
	<xsl:template match="/">
		<div>
			...
			<h4>
				<xsl:value-of select="concat($CircularItemCount + $ProductCount, ' ')"/>products found matching: "<xsl:value-of select="ExpressLane/SearchResults/GlobalSearchResults/GenericProductHelper/SearchText"/>"</h4>
			<p>Organized first by relevance, then alphabetically by department.</p>
	...
	<xsl:template name="CategoryProductDisplayDefault">
		<div>
			...
			<xsl:call-template name="InventoryItems">
				...
				<xsl:with-param name="HeaderText" select="'Top Product Matches'"/>
			</xsl:call-template>
		</div>
	</xsl:template>
                ...
                <xsl:otherwise>
                   <div class="couponsImage">
                        <img>
                            ...
                            <xsl:attribute name="src">
								 /Assets/Common/Images/no_image.jpg
                            </xsl:attribute>
                        </img>
                    </div>
                 </xsl:otherwise>
