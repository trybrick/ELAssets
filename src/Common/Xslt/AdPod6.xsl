...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/" name="AdPodMaster">
		<div class="group leaderboard">
			<xsl:apply-templates select="//ExternalAds">
				<xsl:with-param name="Tile" select="'6'"/>
			</xsl:apply-templates>
		</div>
	</xsl:template>
	<xsl:template name="AdPod" match="ExternalAds">
		...
		<xsl:if test="$Request">
			<div class="{concat('brickunit AdMaster Tile', $Tile)}" data-dimensions="160x600" data-targeting="{concat('{ ', $apos, 'dept', $sep1, $Departments, $sep2, 'kw', $sep1, $PageName, $apos, '}')}" data-store="{concat('', $Zone)}"></div>
			<div id="brxSkyscraper"></div>
		</xsl:if>
	</xsl:template>
</xsl:stylesheet>