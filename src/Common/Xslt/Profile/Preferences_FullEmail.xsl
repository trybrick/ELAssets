...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		...
		<script type="text/javascript">// page specific initialization
		...
		function submitPreferences(){
			...
			<xsl:for-each select="//SignUpV2/Pages/Page">
				<xsl:variable name="id" select="concat('sectionForm', position())"/>sections.push(<xsl:value-of select="concat('&quot;', $id, '&quot;')"/>);</xsl:for-each>var isValid = validateStep();
	...
	<xsl:template name="PageDisplay" match="Page">
		...
		<xsl:variable name="id" select="concat('sectionForm', position())"/>
		<div>
			<xsl:attribute name="id">
				<xsl:value-of select="concat('socialPrefs', position())"/>
			</xsl:attribute>
			<h3 style="cursor:pointer;">
				...
				<xsl:attribute name="onclick">
					<!--<xsl:value-of select="concat('Effect.toggle(&quot;', $id, '&quot;, &quot;blind&quot;, {duration: 1.0});')"/> -->
					<xsl:variable name="display" select="concat('toggleDisplay(&quot;', $id, '&quot;);')"/>
