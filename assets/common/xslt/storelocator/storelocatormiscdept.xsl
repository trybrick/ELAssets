...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:template match="/">
		...
		<script type="text/javascript">
			...
			<xsl:for-each select="//ArrayOfGenericStore/GenericStore[(not(StoreID = //StoreSettings[string-length(StoreGroup) &gt; 0]/StoreID)) and ((StoreID = //StoreSettings[string-length(MiscDeptTitle) &gt; 0]/StoreID) or (StoreID = //StoreSettings[string-length(MiscDeptHours) &gt; 0]/StoreID))]">
			...
			**************************************************************-->
			<div id="StoreLocatorZipLookup">
				...
				<div id="storeZipEntryForm">
					<h2>
						<xsl:choose>
							<xsl:when test="//StoreSettings[string-length(MiscDeptTitle) &gt; 0]/MiscDeptTitle">
								<img alt="" src="{//StoreSettings[string-length(MiscDeptTitle) &gt; 0]/MiscDeptTitle}"/>
								<span> Locations:</span>
							</xsl:when>
							<xsl:otherwise>
	...
	<xsl:template name="StoreGrid" match="GenericStore">
		...
		<div class="storeInfoZip vcard" id="{concat('storeInfoZip:', StoreID)}">
			...
			<a>
				...
				<xsl:choose>
					<xsl:when test="//StoreSettings[string-length(MiscDeptPhone) &gt; 0]/MiscDeptPhone">
						<img alt="" src="{//StoreSettings[string-length(MiscDeptPhone) &gt; 0]/MiscDeptPhone}"/>
					</xsl:when>
					<xsl:otherwise>
