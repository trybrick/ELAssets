		...
		<div id="product-description-lightbox">
			<xsl:for-each select="ExpressLane/GenericProductDescription">
								...
								<table class="nutrition-facts" cellspacing="0" cellpadding="0">
									...
									<tbody>
										...
										<xsl:call-template name="NutrientRow">
											...
											<xsl:with-param name="DisplayIfZero" select="'false'" />
										</xsl:call-template>
										<xsl:call-template name="NutrientRow">
											<xsl:with-param name="GenericNutrient" select="Nutrients/GenericNutrient[normalize-space(Description)='Calories from Fat']" />
											<xsl:with-param name="DisplayIfZero" select="'false'" />
										</xsl:call-template>
										<tr><td colspan="2" class="table-separator"><br /></td></tr>
										<xsl:call-template name="NutrientRow">
										...
										<xsl:call-template name="NutrientRow">
											...
											<xsl:with-param name="IsSubset" select="'true'" />
										</xsl:call-template>
										<xsl:call-template name="NutrientRow">
										...
										<xsl:call-template name="NutrientRow">
											...
											<xsl:with-param name="IsVitaminOrMineral" select="'true'" />
										</xsl:call-template>
										<xsl:call-template name="NutrientRow">
				...
				<xsl:if test="string-length(Directions) &gt; 0">
					<h2>Directions</h2>
					<p><xsl:value-of select="Directions"/></p>
				</xsl:if>
				<xsl:if test="string-length(DrugInteractions) &gt; 0">
					<h2>Drug Interactions</h2>
					<p><xsl:value-of select="DrugInteractions"/></p>
				</xsl:if>
				<xsl:if test="string-length(Warnings) &gt; 0">
					<h2>Warnings</h2>
					<p><xsl:value-of select="Warnings"/></p>
				</xsl:if>
			</xsl:for-each>
		</div>
	</xsl:template>
...
<!-- ========================================================================================== -->
	...
	<xsl:param name="IsVitaminOrMineral" />
	<xsl:param name="DisplayIfZero" />
	<xsl:param name="IsSubset" />
		<xsl:if test="string-length($GenericNutrient/Amount) &gt; 0">
			<tr class="product-nutrition-info amount">
				<xsl:variable name="HasPctDailyValue" select="string-length($GenericNutrient/PctDailyValue) &gt; 0" />
				<td>
					...
					<strong>
						<xsl:attribute name="class">
							<xsl:text>item</xsl:text><xsl:if test="$IsSubset"><xsl:text> sub-nutrient</xsl:text></xsl:if>
						</xsl:attribute>
						<xsl:value-of select="$GenericNutrient/Description" /></strong>
					<xsl:if test="not($IsVitaminOrMineral)">
				...
				<td class="percent-daily">
					<xsl:if test="$HasPctDailyValue and ($DisplayIfZero != 'false')">
