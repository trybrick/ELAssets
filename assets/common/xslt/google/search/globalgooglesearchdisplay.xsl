...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<div>
			...
			<div class="CategoryDisplayContainer">
				<h3 style="cursor:pointer;">
					...
					<xsl:attribute name="onclick">javascript:GSNPageItems.GoogleProductSearch.FilterSearchResults('0');</xsl:attribute>
					<xsl:if test="$catid = 0">
	...
	<xsl:template match="GenericCategory" name="CategoryDisplay">
		...
		<h3 style="cursor:pointer;">
			...
			<xsl:attribute name="onclick">GSNPageItems.GoogleProductSearch.FilterSearchResults('<xsl:value-of select="CategoryID"/>');</xsl:attribute>
	...
	<!-- <xsl:template match="GenericRecipe" name="Recipes">
	<xsl:param name="Recipes" />
	<xsl:param name="HeaderText" />
	
	<xsl:variable name="DefaultHeader" select="'Top Recipes'" />
	<div id="Recipes">
		<xsl:attribute name="class"></xsl:attribute>
		<h2>
			<xsl:choose>
				<xsl:when test="string-length($HeaderText) &gt; 0"><xsl:value-of select="$HeaderText" /></xsl:when>
				<xsl:otherwise><xsl:value-of select="$DefaultHeader" /></xsl:otherwise>
			</xsl:choose>		
		</h2>
		...
		<ul>
			<xsl:for-each select="$Recipes">
				<li>
					<a>
						<xsl:attribute name="href"><xsl:value-of select="concat('/Recipes/RecipeFull.aspx?RecipeID=', RecipeID)" /></xsl:attribute>
