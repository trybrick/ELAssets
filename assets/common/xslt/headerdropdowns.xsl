				...
				<div id="StoreInfoDiv">
					<xsl:choose>
						<xsl:when test="//GenericConsumer/PrimaryStoreID &gt; 0">
							<xsl:variable name="StoreID"><xsl:value-of select="//GenericConsumer/PrimaryStoreID"/></xsl:variable>
							<xsl:call-template name="StoreInformation">
								<xsl:with-param name="PrimaryAddress"><xsl:value-of select="//GenericStore[StoreID = $StoreID]/PrimaryAddress"/></xsl:with-param>
								<xsl:with-param name="SecondaryAddress"><xsl:value-of select="//GenericStore[StoreID = $StoreID]/SecondaryAddress"/></xsl:with-param>
								<xsl:with-param name="City"><xsl:value-of select="//GenericStore[StoreID = $StoreID]/City"/></xsl:with-param>
								<xsl:with-param name="State"><xsl:value-of select="//GenericStore[StoreID = $StoreID]/State"/></xsl:with-param>
								<xsl:with-param name="PostalCode"><xsl:value-of select="//GenericStore[StoreID = $StoreID]/PostalCode"/></xsl:with-param>
								<xsl:with-param name="Phone"><xsl:value-of select="//GenericStore[StoreID = $StoreID]/Phone"/></xsl:with-param>
                                <xsl:with-param name="StoreHours"><xsl:value-of select="//GenericStore[StoreID = $StoreID]/OpenHours"/></xsl:with-param>
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
						<xsl:if test="count(//ArrayOfGenericStore/GenericStore)=1">
							<xsl:call-template name="StoreInformation">
								<xsl:with-param name="PrimaryAddress"><xsl:value-of select="//GenericStore/PrimaryAddress"/></xsl:with-param>
								<xsl:with-param name="SecondaryAddress"><xsl:value-of select="//GenericStore/SecondaryAddress"/></xsl:with-param>
								<xsl:with-param name="City"><xsl:value-of select="//GenericStore/City"/></xsl:with-param>
								<xsl:with-param name="State"><xsl:value-of select="//GenericStore//State"/></xsl:with-param>
								<xsl:with-param name="PostalCode"><xsl:value-of select="//GenericStore/PostalCode"/></xsl:with-param>
								<xsl:with-param name="Phone"><xsl:value-of select="//GenericStore/Phone"/></xsl:with-param>
                                <xsl:with-param name="StoreHours"><xsl:value-of select="//GenericStore/OpenHours"/></xsl:with-param>
							</xsl:call-template>
						</xsl:if>
						</xsl:otherwise>
					</xsl:choose>
