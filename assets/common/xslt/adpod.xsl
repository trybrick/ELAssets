...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="AdPod" match="ExternalAds">
		...
		<xsl:if test="$Request">
			<div class="{concat('brickunit AdMaster Tile', $Tile)}" style="margin: 0 66px;" data-dimensions="728x90" data-targeting="{concat('{ ', $apos, 'dept', $sep1, $Departments, $sep2, 'kw', $sep1, $PageName, $apos, '}')}" data-store="{concat('', $Zone)}"></div>
