	...
	<xsl:template match="StoreCircularPage" name="PageDisplay">
		...
		<img id="CircularImage" style="clear:both;">
			...
			<xsl:if test="number(ImageWidth)">
				<xsl:attribute name="style">width:<xsl:value-of select="ImageWidth"/>px;</xsl:attribute>
			</xsl:if>
		</img>
		...
		<map id="CircularImageMap">
			...
			<xsl:for-each select="Items/StoreCircularItem">
				<xsl:choose>
					<xsl:when test="contains(BrandName,'Kraft')">
						<area target="_new">
							...
							<xsl:attribute name="href">/Ads/Promotion.aspx</xsl:attribute>
