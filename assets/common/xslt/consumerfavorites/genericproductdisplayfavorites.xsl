		...
		<table>
			...
			<tbody>
				<tr>
					...
					<th scope="col" class="header1-2"></th>
...
<xsl:template name="GenericProductTableWithTableIDRelevance">
	...
	-->
		...
		<xsl:variable name="UPC11" select="UPC11" />
		<xsl:variable name="RegularPriceString" select="RegularPriceString"/>
		...
		<xsl:variable name="SmallImage">
			<xsl:choose>
				...
				<xsl:otherwise>
				/Assets/Common/Images/no_image_sm.jpg
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<tr>
			...
			<td class="Favorite">
				...
				<xsl:variable name="FavoriteID" select="concat('favorite_', $ProductID, '_1')"/>
				<xsl:variable name="FavoriteClass">
					<xsl:choose>
						<xsl:when test="($FavoriteProductID &gt; 0) and (string-length($RegularPriceString) &gt; 0) and //GenericConsumer/IsLoggedIn = 'true'">favorite-added</xsl:when>
						<xsl:when test="(string-length($UPC11) &lt; 1) or (string-length($RegularPriceString) &lt; 1)">not-favoritable</xsl:when>
				...
				<xsl:variable name="FavoriteTitle">
					<xsl:choose>
						...
						<xsl:when test="($FavoriteProductID &gt; 0) and (string-length($RegularPriceString) &gt; 0)">
							<xsl:text>This item is a favorite. Click to remove from your favorites.</xsl:text>
						</xsl:when>
						<xsl:when test="(string-length($UPC11) &lt; 1) or (string-length($RegularPriceString) &lt; 1)">
				...
				<xsl:variable name="FavoriteClick">
					<xsl:choose>
						<xsl:when test="(string-length($UPC11) &gt; 0) and (string-length($RegularPriceString) &gt; 0) and //GenericConsumer/IsLoggedIn = 'true'">
			...
			<td>
                ...
                <br/>
					...
					<input type="text" class="comment" id="{$TextBoxID}" maxlength="60">
		...
		<div class="ProductAddToListControl">
			...
			<div>
				...
				<a>
					...
					javascript:AddProductLineItemToList(<xsl:value-of select="ProductID"/>, '<xsl:value-of select="$QuantitySelectID" />', '<xsl:value-of select="$UpdateItemDiv"/>', '<xsl:value-of select="$AddToListDiv"/>');
					SaveComment(<xsl:value-of select="$ProductID"/>, '<xsl:value-of select="$TextBoxID"/>');
					SaveToShoppingListComment(<xsl:value-of select="$ProductID"/>, '<xsl:value-of select="$TextBoxID"/>');
					</xsl:attribute>
					<span>Add</span>
				</a>
			</div>
			<div>
				...
				<a>
					...
					<xsl:attribute name="href">
					javascript:UpdateQuantityInShoppingList(<xsl:value-of select="$ProductID"/>, '<xsl:value-of select="$QuantitySelectID"/>');
					SaveComment(<xsl:value-of select="$ProductID"/>, '<xsl:value-of select="$TextBoxID"/>');
					SaveToShoppingListComment(<xsl:value-of select="$ProductID"/>, '<xsl:value-of select="$TextBoxID"/>');
					</xsl:attribute>
					<span>Update</span>
