		...
		<tr>
			...
			<td class="Favorite">
				...
				<xsl:variable name="FavoriteClass">
					<xsl:choose>
						<xsl:when test="($FavoriteProductID &gt; 0) and (//GenericConsumer/IsLoggedIn = 'true')">favorite-added</xsl:when>
						<xsl:when test="(string-length($UPC11) &lt; 1)">not-favoritable</xsl:when>
				...
				<xsl:variable name="FavoriteTitle">
					<xsl:choose>
						...
						<xsl:when test="(string-length($UPC11) &lt; 1)">
				...
				<xsl:variable name="FavoriteClick">
					<xsl:choose>
						<xsl:when test="(string-length($UPC11) &gt; 0) and (//GenericConsumer/IsLoggedIn = 'true')">
