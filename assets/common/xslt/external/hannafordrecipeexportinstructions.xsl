...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<xsl:for-each select="//GenericRecipe/Instructions/RecipeInstruction">
			<xsl:sort select="DisplayOrder" data-type="number" />
			<div class="recipe_instruction"><xsl:value-of select="concat('', RecipeInstructionDescription)" /></div>
		</xsl:for-each>
	</xsl:template>
