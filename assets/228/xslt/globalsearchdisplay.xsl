...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:import href="/Assets/Common/Xslt/GenericProduct/GenericProductDisplay.xsl"/>
	...
	<xsl:template match="/">
		<div id="SearchResultsHeader">
			...
			<h4>
				<xsl:value-of select="concat($ProductCount, ' ')"/> items found matching: "<xsl:value-of select="ExpressLane/SearchResults/GlobalSearchResults/GenericProductHelper/SearchText"/>"</h4>
			<p>Choose from the list of departments on the left to narrow your search results.</p>
		</div>
		...
		<div>
			...
			<div class="CategoryDisplayContainer">
				<h3 style="cursor:pointer;" id="TopProducts">
	...
	<xsl:template match="GenericCategory" name="CategoryDisplay">
		...
		<xsl:if test="$ProductCount &gt; 0">
			<h3 style="cursor:pointer;">
				...
				<span>(<xsl:value-of select="$ProductCount"/>)</span>
			</h3>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="CategoryProductDisplayDefault">
		<div id="Products" class="CategoryDisplayResultsContainer">
			<!-- <xsl:variable name="SaleItems" select="//Products/GenericProduct[SaleItemID &gt; 0]"/> -->
			<xsl:variable name="InventoryItems" select="//Products/GenericProduct"/>
			
			<!--
			<xsl:call-template name="SaleItems">
				<xsl:with-param name="SaleItems" select="$SaleItems[position() &lt; 4]"/>
				<xsl:with-param name="HeaderText" select="'Top Sale Item Matches'"/>
			</xsl:call-template>
			-->
	...
	<xsl:template match="Category" name="CategoryProductDisplay">
		...
		<xsl:variable name="InventoryItems" select="$CategoryItems"/>
	...
	<xsl:template match="GenericProduct" name="SaleItems">
		...
		<xsl:if test="count($SaleItems) &gt; 0">
			...
			<xsl:call-template name="GenericProductTableRelevance">
				<xsl:with-param name="GenericProducts" select="$SaleItems"></xsl:with-param>
				<xsl:with-param name="AutoSort" select="'false'"></xsl:with-param>
			</xsl:call-template>
		</xsl:if>
